...
var Search = /*#__PURE__*/function () {
  function Search(form) {
    ...
    this.params = querystring.parse(window.location.search); // pre-fill form elements
    ...
    var submitSearchForm = function submitSearchForm(e) {
      e.preventDefault();
      _this.params.q = form.querySelector('input[name="q"]').value;
      _this.params.location = form.querySelector('select[name="location"]').value;
      _this.params.capability = form.querySelector('select[name="capability"]').value;
      history.pushState(null, '', "".concat(window.location.pathname, "?").concat(querystring.stringify(_this.params)));

      _this.renderSearchResults();
    };

    this.submitSearchForm = submitSearchForm.bind(this); // form submit

    form.addEventListener('submit', this.submitSearchForm); // form values change

    form.querySelectorAll('select').forEach(function (input) {
      input.addEventListener('change', _this.submitSearchForm);
  ...
  _createClass(Search, [{
    key: "collectJobsFromDOM",
    value: function collectJobsFromDOM() {
      this.jobs = _toConsumableArray(document.querySelectorAll('[data-search-id]')).map(function (element) {
        return {
          element: element,
    ...
    key: "showNoResultsContainer",
    value: function showNoResultsContainer() {
    ...
    key: "showJobSearchResult",
    value: function showJobSearchResult(job) {
      job.element.style.display = 'block';
    }
  }, {
    key: "hideJobSearchResult",
    value: function hideJobSearchResult(job) {
      job.element.style.display = 'none';
    }
  }, {
    key: "filterJobs",
    value: function filterJobs(jobs, params) {
      var filteredJobs = jobs; // searching by keyword
      ...
      if (params.q) {
        ...
        var index = lunr(function () {
          ...
          jobs.forEach(function (job) {
            return _this2.add(job);
          }, this);
        });
        var results = index.search(params.q);
        var matchingJobIds = results.map(function (result) {
          return result.ref;
        });
        filteredJobs = jobs.filter(function (job) {
          return matchingJobIds.includes(job.id);
        });
      }

      if (params.location) {
        filteredJobs = filteredJobs.filter(function (job) {
          return job.location === params.location;
        });
      }

      if (params.capability) {
        filteredJobs = filteredJobs.filter(function (job) {
          return job.capability === params.capability;
        });
      }
    ...
    value: function renderSearchResults() {
      ...
      var isFilteredSearch = this.params.q && this.params.q.length || this.params.location && this.params.location.length || this.params.capability && this.params.capability.length || null;
      var filteredJobs = this.filterJobs(this.jobs, this.params); // found some matches

      if (isFilteredSearch && filteredJobs.length) {
        this.showResultsContainer();
        this.jobs.forEach(function (job) {
          if (filteredJobs.find(function (j) {
            return j.id === job.id;
          })) {
            _this3.showJobSearchResult(job);
          } else {
            _this3.hideJobSearchResult(job);
          }
        });
      } // no results
          ...
          this.showNoResultsContainer();
            ...
            this.showResultsContainer();
            this.jobs.forEach(function (job) {
              return _this3.showJobSearchResult(job);
            });
